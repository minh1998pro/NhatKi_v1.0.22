package com.example.minh98.diary.model.database
...
class sqlite : SQLiteOpenHelper {
    ...
    constructor(context: Context, name: String, factory: SQLiteDatabase.CursorFactory?, version: Int)
        ...
        , "QueQuan" TEXT
        , "SoThich" TEXT)""")
        queryData("""CREATE  TABLE  IF NOT EXISTS "Diary"
        ("TieuDe" TEXT
        , "NoiDung" TEXT
    ...
    fun insertNhatKi(tieude: String, noidung: String, thoigian: String, taikhoanuser: String): Boolean {
        try {
            queryData("INSERT INTO `Diary` VALUES('$tieude','$noidung','$thoigian','$taikhoanuser')")
            return true
        } catch (e: Exception) {
            return false
        }
    ...
    fun getNhatKiByTaiKhoanUser(taikhoanuser: String): Cursor {
        ...
        val s = "SELECT * FROM `Diary` WHERE `TaiKhoanUser`='$taikhoanuser' ORDER BY `ThoiGian`"
